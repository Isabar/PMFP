...
def write_the_objective_function_init(lingo_model):
   ...
   lingo_model.writelines(f'!Objective function;\n')
   lingo_model.writelines(f'[obj] Min=@sum(Clients(i): demand(i)*EC(i));\n\n')
   
   return 

def write_the_objective_function_CMS(lingo_model):

   lingo_model.writelines(f'!Objective function;\n')
   lingo_model.writelines(f'[obj] Min=@sum(Clients(i): demand(i)*EC(i))+@sum(Facilities(k):Cap(k));\n\n')
   
   return 

def write_the_objective_function_CML(lingo_model):

   lingo_model.writelines(f'!Objective function;\n')
   lingo_model.writelines(f'[obj] Min=@sum(Clients(i): demand(i)*EC(i))+@sum(Facilities(k):perte(k));\n\n')
...
def write_capacity_lost_constraints(lingo_model):
   ...
   lingo_model.writelines(f'! Capacity constraints;\n') 
   lingo_model.writelines(f'@for (Clients(i): @for( Facilities(k): P(i,k)=@prod(SORTED(i,k2)|k2 #LT# positions(i,k):@sum(Levels(l):proba(sort(i,k2),l)*z(sort(i,k2),l)))*@sum(Levels(l2):(1-proba(positions(i,k),l2))*z(positions(i,k),l2))));\n')
   lingo_model.writelines(f'@for(Facilities(j):@sum(Clients(i):P(i,j)*demand(i))<= Cap(j)+perte(j));\n\n')
