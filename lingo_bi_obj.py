# -*- coding: utf-8 -*-
"""
Created on Mon Jul 11 15:35:03 2022

@author: baret
"""

def create_lingo_bi_obj(directory, instance_number,model, relaxed,cap,typeProba):
   ...
   write_constraints_init(lin_model, 1)
...
def write_sets(instance_folder_path, instance,lingo_model):
   ...
   lingo_model.writelines(f'Clients: demand, EC, penalty, H, perte;\n')
   lingo_model.writelines(f'Facilities: Cap, Cmin;\n')
...
def write_results_data(instance_folder_path,instance_number,lingo_model,relaxed,model):
   ...
   lingo_model.writelines(f'@ole(\'{results_folder_path}{results}\',\'F3\')=@WRITE(\'P1\');\n')
   lingo_model.writelines(f'@ole(\'{results_folder_path}{results}\',\'F4\')=@WRITE(P1);\n')
   lingo_model.writelines(f'@ole(\'{results_folder_path}{results}\',\'G3\')=@WRITE(\'P2\');\n')
   lingo_model.writelines(f'@ole(\'{results_folder_path}{results}\',\'G4\')=@WRITE(P2);\n')
   return 
...
#   lingo_model.writelines(f'H2=(@sum(Clients(i):@prod(SORTED(i,k3)|distance(i,k3) #LT# 5:@sum(Levels(l2):proba(sort(i,k3),l2)*z(sort(i,k3),l2)))*penalty(i)*demand(i)));\n')
   
    lingo_model.writelines(f'@for(Clients(i):H(i)=demand(i)*@sum(SORTED(i,k1)|distance(i,k1) #LT# 50:@prod(SORTED(i,k2)|k2 #LT# k1:@sum(Levels(l1): proba(sort(i,k2),l1)*z(sort(i,k2),l1)))* @sum(Levels(l):(1-proba(sort(i,k1),l))*z(sort(i,k1),l))*distance(i,k1)));\n')
    lingo_model.writelines(f'MaxH=@max(Clients: H);\n')
    lingo_model.writelines(f'H1=@sum(Clients(i):((H(i))/MaxH)*100);\n');
    
    lingo_model.writelines(f'@for(Clients(i): Perte(i)=@prod(SORTED(i,k3)|distance(i,k3) #LT# 50:@sum(Levels(l2):proba(sort(i,k3),l2)*z(sort(i,k3),l2)))*penalty(i)*demand(i));\n')
    lingo_model.writelines(f'MaxP=@max(Clients:Perte);\n')
    lingo_model.writelines(f'H2=@sum(Clients(i):((Perte(i))/MaxP)*100);\n')
